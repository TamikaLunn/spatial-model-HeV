...
## Meta-population specifications:
## Transmission is density dependent
## Transmission in the meta-population is between groups (denoted i and j).
## Bij is the rate at which susceptible individuals in the ith tree group and infectious individuals in the jth tree group come into affective contact per unit time
...
## model structures have no birth/death or immigration/emigration - is a completely closed roost

## Call to functions
#setwd("D:/PhD/Project/Manuscripts/First author/Planned_Roost structure #3 (density and disease)/Roost disease R")
#source ("stoch_helperfunctions_V18.R")
...
##---------------------------------Function: define the SIRS model structure---------------------------------
SIRSstoch <- function(state, params, transitions) { 
  ## Define parameters
  n <- params$n
  B <- params$beta
  gamma <- params$gamma
...
##---------------------------------Function: define the SEIRS model structure---------------------------------
SEIRSstoch <- function(state, params, transitions) { 
  ## Define parameters
  n <- params$n
  B <- params$beta
  gamma <- params$gamma
  ...
  rates <- c(
    exposure = colSums(B*I)*S,
    infection = delta*E, #note: infection needs to be first rate specified for infections tally
    recovery=gamma*I, #returns a vector of infection and recovery rate per group
    waning = sigma*R
  )
  ## Define event selection and rate
