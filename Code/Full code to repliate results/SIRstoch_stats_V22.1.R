## Title: R script to run functions for *SIR* stochastic compartmental model with gillespie algorithm
## Author: Tamika Lunn, Griffith University
## Version: V22, created 5th May 2020

rm(list=ls())
### Set values for visualisation:
threshold = 10 #set number of infections needed to be considered a successful outbreak 
modelrun <- "SIR_stoch outputs_PO_p50" #change to whichever model set you are creating figures for (name as specified in file directory)
...
modstrs <- "SIRns|SIR" #SIR = with spatial structure, SIRns = without spatial structure (homogeneous model)
colnames <- c("modstr","Bfun","site","id","subplot","n","Beta","gamma","N","Ntot","radius","theta","threshold","nsims","R0roost")

setwd(paste(rprojroot::find_rstudio_root_file(),"/Data/Processed",sep=""))
structure <- read.csv("plot groups.csv", row.names=1, check.names=FALSE) #extra information on data matrices
...
spatmodstr <- "SIR" #to subset runvalues by spatial model structure 
nonspatmodstr <- "SIRns" #to subset runvalues by spatial model structure

### Merge by Beta (combine all theta):
unique(series.params$Beta)
param.values <- unique(series.params$Beta)
...
successoverview <- ddply(simend.output.merge[[1]], c("modstr","structure", "R0roost", "Beta", "theta", "Ntot", "nsims"), summarise,
                         ...
                         median.ext.time = median(end.time),
                         LIQR.ext.time = Liqr(end.time),
                         UIQR.ext.time = Uiqr(end.time)) %>%
...
## Winter
ext.wnt.filename <- "extinction-winter_structure with fixed Ntot and theta values_SIR" 
...
## Year
ext.yr.filename <- "extinction-year_structure with fixed Ntot and theta values_SIR" 
...
## Plot
spread.filename <- "spread-discrete_structure with fixed Ntot and theta values_SIR"
...
## Plot
inf.filename.wnt <- "infection-discrete-winter_structure with fixed Ntot and theta values_SIR" 
inf.filename.yr <- "infection-discrete-year_structure with fixed Ntot and theta values_SIR"
...
line_ribbon_plot(summary.output.merge.discrete.med, spatmodstr, nonspatmodstr, "median.I.prop", "time", "LIQR", "UIQR", "structure", "theta", "Ntot", inf.filename.wnt, ylab, xlab, mainlab, 90, 0.4)
line_ribbon_plot(summary.output.merge.discrete.med, spatmodstr, nonspatmodstr, "median.I.prop", "time", "LIQR", "UIQR", "structure", "theta", "Ntot", inf.filename.yr, ylab, xlab, mainlab, 365, 0.4)
#----------------------------------------------------------------------------------------#
...
time.box.filename <- "time to epidemic peak_structure with fixed Ntot and theta values_SIR"
ylab <- "Time of epidemic peak (days)"  
xlab <- "Stand structure"
mainlab <- "Time of epidemic peak"
max(epipeak.merge[[1]]$time) #check max y value
box_plot_combined(epipeak.merge, spatmodstr, nonspatmodstr, "time", "Group", "theta", "Ntot", time.box.filename, ylab, xlab, mainlab, 200) #spatial and non-spatial models

## Magnitude of epidemic peak:
mag.box.filename <- "magnitude of epidemic peak_structure with fixed Ntot and theta values_SIR"
...
duration.box.filename <- "duration of epidemic peak_structure with fixed Ntot and theta values_SIR"
ylab <- "Duration of epidemic peak (days)"  
xlab <- "Stand structure"
mainlab <- "Duration of epidemic peak"
max(epiduration[[1]]$duration) #check max y value
box_plot_combined(epiduration, spatmodstr, nonspatmodstr, "duration", "Group", "theta", "Ntot", duration.box.filename, ylab, xlab, mainlab, 175)
...
## Plot
inftree.disc.filename.wnt <- "By trees infected-discrete time-winter_structure with fixed Ntot and theta values_SIR"
inftree.disc.filename.yr <- "By trees infected-discrete time-year_structure with fixed Ntot and theta values_SIR" 
...
# -------------Time to extinction------------- #
## Average per bar
filename <- "SIR_extinction time_by bar"
grouplist <- c("modstr","structure", "plotid", "R0roost", "Beta", "theta", "Ntot", "nsims")
Nsuccessful <- ext.time.summary(simend.output.merge, grouplist, filename) #this code gives number of successful outbreaks, to calculate proportion of successful outbreaks below. If the origional dataframe has more than 1 list, the way this is saved will need to change to accomadate multiple lists

## Average per tree structure
filename <- "SIR_extinction time_by tree"
...
## Value per bar
Nsuccessful$prop.successful <- Nsuccessful$count.n/Nsuccessful$nsims #smallest level for the proportion of successful outbreaks (because calculated from all simulations per simulation run)

## Average per tree structure
filename <- "SIR_prop extinction_by tree.csv"
grouplist <- c("modstr", "structure", "plotid", "Beta", "theta", "nsims")
success.summary(Nsuccessful, grouplist, filename) 

## Average per abundance
filename <- "SIR_prop extinction_by Ntot.csv"
grouplist <- c("Ntot", "Beta", "theta", "nsims")
success.summary(Nsuccessful, grouplist, filename) 

## Average per model structure
filename <- "SIR_prop extinction_by modstr.csv"
grouplist <- c("modstr", "Beta", "theta", "nsims")
success.summary(Nsuccessful, grouplist, filename) 

## Average per model structure & population
filename <- "SIR_prop extinction_by modstr & Ntot.csv"
grouplist <- c("modstr", "Ntot", "Beta", "theta", "nsims")
success.summary(Nsuccessful, grouplist, filename) 
# -------------------------------------------- #
...
## Average per box
filename <- "SIR_epidemic mag and time_by bar"
...
filename <- "SIR_epidemic mag and time_by Group"
grouplist <- c("Group", "Beta", "theta", "nsims")
epi.MagTime.summary(epipeak.merge, grouplist, filename) 

## Average per abundance
filename <- "SIR_epidemic mag and time_by Ntot"
grouplist <- c("Ntot", "Beta", "theta", "nsims")
epi.MagTime.summary(epipeak.merge, grouplist, filename) 

## Average per model structure
filename <- "SIR_epidemic mag and time_by modstr"
grouplist <- c("modstr", "Beta", "theta", "nsims")
epi.MagTime.summary(epipeak.merge, grouplist, filename) 

## Average per model structure & population
filename <- "SIR_epidemic mag and time_by modstr & Ntot"
grouplist <- c("modstr", "Ntot", "Beta", "theta", "nsims")
epi.MagTime.summary(epipeak.merge, grouplist, filename) 
# -------------------------------------------- #
...
## (note - repace tree structure with Group to get both spatial and non-spatial comparisons)
## Average per box
filename <- "SIR_epidemic duration_by box"
...
## Average per Group structure
filename <- "SIR_epidemic duration_by Group"
grouplist <- c("Group", "Beta", "theta")
epi.duration.summary(epiduration.n, grouplist, filename) 

## Average per abundance
filename <- "SIR_epidemic duration_by Ntot"
grouplist <- c("Ntot", "Beta", "theta")
epi.duration.summary(epiduration.n, grouplist, filename) 

## Average per model structure
filename <- "SIR_epidemic duration_by modstr"
grouplist <- c("modstr", "Beta", "theta")
epi.duration.summary(epiduration.n, grouplist, filename) 

## Average per model structure & population
filename <- "SIR_epidemic duration_by modstr & Ntot"
...
for (i in 1:length(summary.output.merge.discrete.med.proptrees)) {
  ...
  write.csv(Itreesummary, paste("SIR_days with max infected trees_list ", i, ".csv",sep="")) #save summary for later reference
}
# -------------------------------------------- #
...
for (i in 1:length(R0tree.merge)) {
  R0tree.table[[i]] <- ddply(R0tree.merge[[i]], c("modstr","structure", "theta", "R0roost", "Beta", "theta", "Ntot", "nsims", "index"), summarise,
