...
# Transition matrices for each SIR or SEIR model structure type
...
Data <- 1:length(DATA) #number of dfs in list (i.e. number of stand structures being compared)
...
modstr <- c("SIR", "SIRns") #SIR = with spatial structure, SIRns = without spatial structure (homogeneous model)
prev <- 0.5 #start population seroprevalence. Repeated for values 0, 50 and 75 in the paper
...
transitions.fun <- transitions.SIR.fun #function for defining transition matrix ***
...
fun.index.simulation <- SIR.index.simulation #function for choosing index case per simulation set. Don't need to change for SIRS (no impact of S on SIR groups)
fun.summary <- SIRsummary #function for creating summary data from simulation output. Don't need to change for SIRS (no impact of S on SIR groups)
uni.simulation <- SIR.uni.simulation.time #function for defining when to stop simulation ***
stoch.fun <- SIRstoch #function for main program ***
...
R0roost <- 1/prev #calculate R0 when equilibrium
paramvalues <- list(N=N, Ntot=Ntot, Data=Data, radius=radius, theta=theta, R0roost=R0roost, gamma=gamma, nsims=nsims, threshold=threshold, modstr=modstr) #note - need to specify N as first and Ntot as second elements in the list, for indexing within function ***
...
### Set beta matrices for data. Change line depending on which function used (gravity or alternative)
...
## Set values based on values in runvalues
  ...
  scaled.data.alt <- SCALED.DATA.ALT[[i]] #used for spread functionradius <- runvalues$radius[i] 
  ...
  params <- list(n=n, beta=B,gamma=gamma)
  ...
  I=rep(0,n)
  R=Ntree-S
  start <- c(S=S, I=I, R=R,time=0, index=0, infections=0) #initial conditions ***
  ...
  ## Define conditions for model
  maxstep = 500000 #doesn't influence ending of simulation when .time function used, but it does need to be > the number of rows expected (get to about 59,000 with SEIR at 365 days)
  endtime = 365
  ...
  ##---------------------------------Save outputs---------------------------------
  param.labels <- paste('id=',id,' ',modstr,' ', name,' ',plotid,' n=',n,' B=',format(round(Beta,digits=6), scientific=FALSE),' g=',format(round(gamma,digits=4), scientific=FALSE),' N=',N,' Nt=',Ntot,' rad=',radius, ' tht=',theta,' thrsh=',threshold,' nsims=',nsims,' RO=',round(R0roost, digits=3),sep='')
