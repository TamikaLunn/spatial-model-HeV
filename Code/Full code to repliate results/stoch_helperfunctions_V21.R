...
## Version: V21, created 9th June 2021
## - Current version includes amendments to figures as suggested by reviewers for Journal of Animal Ecology
...
library(igraph) #for network
library(data.table) #for rolling join
...
##----------------------Function(s): thin a single simulation-----------------------
...
##---------------------------------Function: extract values from end of simualtion. Use to create list of successful simulations---------------------------------
...
##---------------------------Function: create parameter table for running values with fixed R0roost-------------------
...
plot.ext.group <- function(data, fixedparam, subsetparam, group, filename, collabels) {
  ...
  for (i in 1:length(data)) {
    ...
    plots.bar[[i]] <- data.sub[[i]] %>%
      ...
      geom_bar(position="fill", stat="identity")+
      labs(y="Proportion of simulations resulting in outcome", x="Stand structure", fill="Outcome of simulation") +
...
line_ribbon_plot <- function(dataset, spatmodstr, nonspatmodstr, yname, xname, Lvarname, Uvarname, group, fixedparam, subsetparam, filename, ylab, xlab, mainlab, xlim, ylim) {
  ...
  for (i in 1:length(dataset)) {
    ...
    spread.plots[[i]] <- ggplot() +
      ...
      geom_line(data=dataspat[[i]], aes(y=dataspat[[i]][[yname]], x=dataspat[[i]][[xname]], colour=dataspat[[i]][[group]]), size=0.8, linetype="solid") + #geom_line is not a single, good looking line - needs to have only one combination set plotted to be this. Is likely not ok to loess smooth my already smoothed lines... 
      geom_ribbon(data=dataspat[[i]], aes(ymin=dataspat[[i]][[Lvarname]], ymax=dataspat[[i]][[Uvarname]], x=dataspat[[i]][[xname]], group=dataspat[[i]][[group]]), alpha=0.35, fill = "grey70") +
      
      #non spatial (dashed line, transparant labels)
      geom_line(data=datanspat[[i]], aes(y=datanspat[[i]][[yname]], x=datanspat[[i]][[xname]], colour=datanspat[[i]][[group]]), size=0.8, linetype="dashed") + #geom_line is not a single, good looking line - needs to have only one combination set plotted to be this. Is likely not ok to loess smooth my already smoothed lines... 
      geom_ribbon(data=datanspat[[i]], aes(ymin=datanspat[[i]][[Lvarname]], ymax=datanspat[[i]][[Uvarname]], x=datanspat[[i]][[xname]], group=datanspat[[i]][[group]]), alpha=0.35, fill = "grey70") +
...
box_plot_combined <- function(dataset, spatmodstr, nonspatmodstr, yname, group, fixedparam, subsetparam, filename, ylab, xlab, mainlab, ylim) {
  ...
  for (i in 1:length(dataset)) {
    ...
    epipeak.plots[[i]] <- ggplot() +
      ...
      geom_point(data=data[[i]], aes(y=data[[i]][[yname]], x=data[[i]][[group]], colour=data[[i]][[group]]), alpha=0.2) +
